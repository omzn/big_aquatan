...
//  pan_servo = p;
  tilt_servo = t;
  _tilt_center = (TILT_MAX_DEG + TILT_MIN_DEG) / 2;
  _slowtilt_moving = 0;
  _tilt_interval = 10;
}
...
void AquatanHead::setMinMax(int16_t tmin, int16_t tmax) {
  _tilt_min_deg = tmin;
  _tilt_max_deg = tmax;
  _tilt_center = (_tilt_max_deg + _tilt_min_deg) / 2; 
}

void AquatanHead::tilt(int16_t deg) { // 
  Serial.print("deg:");
  Serial.println(deg);
  _tilt_deg = constrain(deg, _tilt_min_deg - _tilt_center, _tilt_max_deg - _tilt_center );
  Serial.print("_tilt_deg:");
  Serial.println(_tilt_deg);
  tilt_servo->write( _tilt_center + (_tilt_deg * TILT_DIRECTION));
  Serial.print("Servo:");
  Serial.println(tilt_servo->read());
}

void AquatanHead::tiltRatio(int16_t ratio) {
  Serial.print("ratio:");
  Serial.println(ratio);
  tilt(map(ratio,-100,100, _tilt_min_deg - _tilt_center, _tilt_max_deg - _tilt_center));
}

int16_t AquatanHead::tilt() { return _tilt_deg; }

int16_t AquatanHead::tiltRatio() { return map(_tilt_deg, _tilt_min_deg - _tilt_center, _tilt_max_deg - _tilt_center, -100, 100); }

uint8_t AquatanHead::setSlowTilt(int16_t ratio) {
  if (!_slowtilt_moving) {
    _slowtilt_moving = 1;
    int16_t convdeg = map(ratio, -100, 100, _tilt_min_deg - _tilt_center, _tilt_max_deg - _tilt_center );
    _slowtilt_target_deg = constrain(convdeg, _tilt_min_deg - _tilt_center, _tilt_max_deg - _tilt_center);
    return 1;
  } else {
    return 0;
  }
